...
class sctp_socket_ogs
{
public:
  ogs_sock_t *sock_ptr;
  static expected<sctp_socket_ogs> create(const sctp_socket_ogs_params& params);

  sctp_socket_ogs();
  sctp_socket_ogs(sctp_socket_ogs&& other) noexcept = default;
  ~sctp_socket_ogs();
  sctp_socket_ogs& operator=(sctp_socket_ogs&& other) noexcept;

  bool close();

  [[nodiscard]] bool is_open() const { return sock_ptr != nullptr; }
  const unique_fd&   fd() const { return sock_fd; }
  const ogs_sock_t*  ptr() const { return sock_ptr; }
...
private:
  bool set_sockopts(const sctp_socket_ogs_params& params);
...
namespace fmt {
template <>
struct formatter<srsran::sctp_socket_ogs_params> {
  ...
  auto format(const srsran::sctp_socket_ogs_params& cfg, FormatContext& ctx)
  {
