...
/// user space SCTP socket instance.
class sctp_socket_usr
{
public:
  static expected<sctp_socket_usr> create(const usrsctp_socket_params& params);

  sctp_socket_usr();
  sctp_socket_usr(sctp_socket_usr&& other) noexcept = default;
  ~sctp_socket_usr();
  sctp_socket_usr& operator=(sctp_socket_usr&& other) noexcept;

  bool close();

  SRSRAN_NODISCARD bool is_open() const { return sock_ptr != nullptr; }
  const struct socket*      ptr() const { return sock_ptr; }
...
private:
  bool set_sockopts(const usrsctp_socket_params& params);
...
namespace fmt {
template <>
struct formatter<srsran::usrsctp_socket_params> {
  ...
  auto format(const srsran::usrsctp_socket_params& cfg, FormatContext& ctx)
  {
